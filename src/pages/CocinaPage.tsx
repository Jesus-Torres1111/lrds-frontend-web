...
import { 
  ...
  getRecetaProducto, type KdsPedido, type PorcionDisponible 
} from '@/api/kds';
...
const THEMES = {
  light: {
    appBg: 'bg-zinc-100', modalBg: 'bg-white', textMain: 'text-zinc-900', textMuted: 'text-zinc-500',
    border: 'border-zinc-200', borderDark: 'border-zinc-300', btnGhost: 'bg-white hover:bg-zinc-100 text-zinc-700 border-zinc-200 shadow-sm',
    itemBg: 'bg-white border-zinc-200', activeTab: 'bg-zinc-900 text-white shadow-md'
  },
  dark: {
...
function ModalReceta({ isOpen, onClose, productoId, nombreProducto, theme }: any) {
  ...
  return (
    <div className="fixed inset-0 bg-black/90 backdrop-blur-sm flex items-center justify-center z-50 p-4 sm:p-6 animate-in fade-in duration-200">
      <div className={`${c.modalBg} border ${c.border} rounded-[2rem] shadow-2xl w-full max-w-4xl flex flex-col max-h-[85vh] overflow-hidden`}>
        
        <div className={`px-6 py-5 border-b ${c.border} flex justify-between items-center shrink-0`}>
          <div className="flex items-center gap-3">
            <div className="bg-blue-500/20 p-2.5 rounded-xl"><Info className="text-blue-500 w-6 h-6" /></div>
            <h2 className={`${c.textMain} font-black text-2xl tracking-tight`}>Ficha Técnica</h2>
          </div>
          <button onClick={onClose} className={`${c.btnGhost} p-2 rounded-xl active:scale-95 transition-all`}><X size={20} /></button>
        </div>
        
        <div className="flex flex-col md:flex-row flex-1 min-h-0 overflow-hidden">
          <div className={`w-full md:w-1/2 p-6 md:border-r ${c.border} flex flex-col`}>
            {!loading && receta?.imagenUrl ? (
              <div className="w-full h-full min-h-[250px] md:min-h-full rounded-[1.5rem] overflow-hidden border border-gray-700 shadow-inner bg-black">
                <img src={receta.imagenUrl} alt={nombreProducto} className="w-full h-full object-cover" />
              </div>
            ) : (
              <div className={`w-full h-full min-h-[250px] md:min-h-full rounded-[1.5rem] border-2 border-dashed ${c.borderDark} flex flex-col items-center justify-center text-gray-500`}>
                <UtensilsCrossed size={48} className="mb-4 opacity-30" />
                <p className="font-bold text-sm">Sin imagen de referencia</p>
              </div>
            )}
          </div>

          <div className="w-full md:w-1/2 p-6 overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
            <h3 className={`${c.textMain} text-3xl font-black mb-6 tracking-tight`}>{nombreProducto}</h3>
            
            {loading ? (
              <div className="flex flex-col items-center justify-center py-20 text-gray-500">
                <RefreshCcw className="animate-spin w-10 h-10 mb-4" />
                <p className="font-bold">Cargando receta...</p>
              </div>
            ) : receta ? (
              <div className="space-y-8 pb-4">
                <div>
                  <p className={`text-xs font-black uppercase tracking-widest ${c.textMuted} mb-3`}>Ingredientes Estándar</p>
                  <div className="space-y-2.5">
                    {receta.ingredientes?.map((ing: any, i: number) => (
                      <div key={i} className={`flex justify-between items-center p-3.5 rounded-xl border ${c.border} ${c.itemBg} shadow-sm`}>
                        <span className={`${c.textMain} font-bold text-[15px]`}>{ing.insumo}</span>
                        <span className={`${c.textMuted} font-black text-sm bg-gray-500/10 px-3 py-1 rounded-lg`}>{ing.cantidad} {ing.unidad}</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                {receta.instrucciones && (
                  <div>
                    <p className={`text-xs font-black uppercase tracking-widest ${c.textMuted} mb-3`}>Preparación Paso a Paso</p>
                    <div className={`p-5 rounded-2xl border ${c.border} ${c.itemBg} shadow-sm flex flex-col gap-3`}>
                      {receta.instrucciones.split('\n').map((paso: string, idx: number) => (
                        <p key={idx} className={`${c.textMain} text-sm leading-relaxed m-0`}>
                          {paso}
                        </p>
                      ))}
                    </div>
              ...
              <div className="text-center py-20">
                <AlertTriangle className="mx-auto text-amber-500 w-16 h-16 mb-4 opacity-50" />
                <p className={`${c.textMain} font-black text-xl mb-1`}>Receta no vinculada</p>
                <p className={`${c.textMuted} font-medium text-sm`}>Contacta al administrador para configurar los ingredientes.</p>
              </div>
            )}
          </div>
        </div>
...
interface TicketPedidoProps {
  pedido: KdsPedido;
  onPreparar: (id: number) => void;
  onListo: (id: number) => void;
  onVerReceta: (id: number, nombre: string) => void;
  procesando: boolean;
  theme: ThemeKey;
}

const TicketPedido = React.memo(function TicketPedido({ pedido, onPreparar, onListo, onVerReceta, procesando, theme }: TicketPedidoProps) {
  ...
  const configMap: Record<EstadoTicket, { colorBorder: string; colorText: string; badge: string; bgTime: string }> = {
    URGENTE: { colorBorder: 'border-red-600/80', colorText: 'text-red-500', badge: 'URGENTE', bgTime: 'bg-red-900/30' },
    DEMORADO: { colorBorder: 'border-amber-500/80', colorText: 'text-amber-500', badge: 'DEMORADO', bgTime: 'bg-amber-900/20' },
    EN_PROCESO: { colorBorder: 'border-orange-500/80', colorText: 'text-orange-500', badge: 'EN PROCESO', bgTime: 'bg-orange-900/20' },
    NUEVO: { colorBorder: 'border-[#FFC640]/70', colorText: 'text-[#FFC640]', badge: 'NUEVO', bgTime: 'bg-[#FFC640]/10' }
  };
          ...
          {pedido.items.map((item) => (
            <div key={item.detalleId} className={`rounded-xl p-3.5 group relative ${isDark ? 'bg-[#141414]' : 'bg-gray-50 border border-gray-100'}`}>
              <div className="flex justify-between items-start">
                ...
                <button onClick={() => onVerReceta(item.productoId, item.producto)} aria-label="Ver Receta" className="opacity-0 group-hover:opacity-100 text-blue-500 hover:text-blue-400 transition-opacity p-1" title="Ver Receta">
  ...
  if (prevProps.procesando !== nextProps.procesando) return false;
  if (prevProps.theme !== nextProps.theme) return false;
  if (prevProps.pedido.estadoPedido !== nextProps.pedido.estadoPedido) return false;
  if (prevProps.pedido.items.length !== nextProps.pedido.items.length) return false;
  
  if (Math.floor(prevProps.pedido.minutosTranscurridos) !== Math.floor(nextProps.pedido.minutosTranscurridos)) return false;
...
export default function CocinaPage() {
  ...
  const handlePreparar = useCallback(async (pedidoId: number) => {
    ...
    <div className={`h-screen flex flex-col font-sans overflow-hidden transition-colors duration-300 ${c.appBg}`}>
      ...
      <header className={`border-b px-6 py-4 flex flex-col xl:flex-row items-center justify-between z-30 shrink-0 gap-4 ${theme === 'dark' ? 'bg-[#0a0a0a] border-gray-800' : 'bg-white border-gray-200'}`}>
        ...
        <div className="flex flex-wrap items-center gap-3 w-full xl:w-auto justify-center">
          ...
          <button onClick={() => setModalAgotados(true)} aria-label="Pizarra de Agotados" className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-amber-500/10 text-amber-500 hover:bg-amber-500/20 border border-amber-500/20 font-bold text-xs transition-colors flex-shrink-0">
            <AlertTriangle size={16} strokeWidth={2.5} /> <span className="hidden sm:inline">Pizarra 86</span>
          </button>
          
          <button onClick={() => changeTheme(theme === 'light' ? 'dark' : 'light')} aria-label="Cambiar tema oscuro o claro" className={`p-2.5 rounded-xl border flex-shrink-0 transition-colors ${theme === 'dark' ? 'bg-[#141414] text-gray-400 border-gray-800 hover:text-white' : 'bg-white text-gray-500 border-gray-200 hover:text-black'}`}>
        ...
        <div className="flex items-center gap-4 w-full xl:w-auto justify-center xl:justify-end shrink-0">
          ...
          <button onClick={() => { logout(); navigate('/login'); }} aria-label="Cerrar sesión" className={`p-2.5 rounded-xl transition-colors ${theme === 'dark' ? 'text-gray-500 hover:text-rose-500 hover:bg-rose-500/10' : 'text-gray-500 hover:text-rose-600 hover:bg-rose-50'}`} title="Cerrar Sesión">
            <LogOut size={20} />
          </button>
        </div>
