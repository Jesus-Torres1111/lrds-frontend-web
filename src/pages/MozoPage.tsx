...
import { 
  ...
  Trash2, ShieldAlert, Moon, Sun, RefreshCw, CalendarDays
} from 'lucide-react';
import { 
...
const THEMES = {
  light: {
    ...
    primaryBtn: 'bg-gradient-to-r from-orange-500 to-amber-500 hover:from-orange-400 hover:to-amber-400 text-white font-black font-bold', secondaryBtn: 'bg-zinc-100 hover:bg-zinc-200 text-zinc-700',
...
function ModalAgregarItems({ theme, pedido, productos, categorias, onClose, onAgregado }: any) {
  ...
  const disponibles = productos.filter((p: any) => {
    ...
    const matchBusqueda = p.nombre.toLowerCase().includes(busqueda.toLowerCase()) || 
                          (p.tagsBusqueda && p.tagsBusqueda.toLowerCase().includes(busqueda.toLowerCase()));
  ...
  const handleEnviar = async () => {
    ...
    try {
      ...
      sileo.error({ title: 'Error al agregar', description: <span className="text-white">{err.response?.data?.message || err.message}</span> });
    } finally {
      setEnviando(false);
    }
  };
            ...
            <div className={`p-4 border-b ${c.border} shrink-0`}>
              <div className="relative mb-3">
                ...
                <input 
                  ...
                  placeholder="Buscar platillo (ej: picante, bebida)..." 
...
function HeaderMozo({ user, theme, changeTheme, vistaMovil, setVistaMovil, notificaciones, setModalAbierto, handleLogout, goToReservas }: any) {
...
function PanelNuevaOrden({ theme, vistaMovil, paso, setPaso, tipoConsumo, setTipoConsumo, mesa, setMesa, modoMesaCustom, setModoMesaCustom, carrito, setCarrito, notasGenerales, setNotasGenerales, enviando, enviarPedido, productos, categorias, pedidos }: any) {
  ...
  const seleccionarMesa = (num: string) => {
    ...
    if (ocupada) return sileo.error({ title: 'Mesa Ocupada', description: <span className="text-white">Ya existe un pedido activo en esta mesa</span> });
    ...
    <div className={`w-full md:w-[460px] xl:w-[500px] ${c.panelBg} flex flex-col z-10 border-r ${c.border} shrink-0 min-h-0 transition-colors ${vistaMovil === 'NUEVA_ORDEN' ? 'flex' : 'hidden md:flex'}`}>
      ...
      {paso === 'MENU' && (
        <div className="flex flex-col flex-1 overflow-hidden relative">
          ...
          <div className={`flex-1 overflow-y-auto p-4 space-y-3 pb-24 ${c.appBg} custom-scrollbar`}>
                  ...
                  <div className={`px-6 py-5 border-b ${c.border} flex justify-between items-start`}>
                    ...
                    <div className="flex flex-col items-end gap-2">
                      ...
                      {pedido.estadoActual !== 'CANCELADO' && pedido.estadoActual !== 'PAGADO' && pedido.estadoActual !== 'ENTREGADO' && (
                        <button onClick={() => handleCancelarPedido(pedido.id)} className={`text-xs font-bold ${c.textMuted} hover:text-red-500 transition-colors flex items-center gap-1`} title="Anular Orden">
...
useEffect(() => {
  ...
  const enviarPedido = async () => {
    ...
    } catch (err: any) { sileo.error({ title: 'Error', description: <span className="text-white">{err.response?.data?.message || err.message}</span> }); } 
  ...
  const handleEntregar = async (id: number) => {
    try { 
       ...
       sileo.error({ title: 'No se puede entregar', description: <span className="text-white">{err.response?.data?.message || err.message}</span> }); 
    }
  };
  ...
  const handleCancelarItem = (pId: number, dId: number, estado: EstadoItem) => {
    ...
    setConfirmDialog({
      ...
      onConfirm: async (m?: string) => {
        ...
        catch (err: any) { sileo.error({ title: 'Error', description: <span className="text-white">{err.message}</span> }); } 
  ...
  return (
    <div className={`h-screen flex flex-col font-sans overflow-hidden transition-colors duration-300 ${c.appBg} relative`}>
      <HeaderMozo user={user} theme={theme} changeTheme={changeTheme} vistaMovil={vistaMovil} setVistaMovil={setVistaMovil} notificaciones={notificaciones} setModalAbierto={setModalAbierto} handleLogout={() => { logout(); navigate('/login'); }} goToReservas={() => navigate('/reservas')} />
